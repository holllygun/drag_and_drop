export default class Tables {
  ...
  deleteTask() {
    document
      .querySelector(".planner_container")
  ...
  newTask(input, btn, popupBtn) {
    ...
    if (!value) {
      ...
      taskContainer.insertAdjacentHTML(
        "afterbegin",
        `<div class="wrapper"><div class="task">${value}</div><button class="delete_btn">⨉</button></div>`,
      );
      input.remove();
    ...
    addBtns.forEach((btn) => {
      ...
      btn.addEventListener("click", () => {
        ...
        btn.insertAdjacentHTML(
          "beforebegin",
          `<input class="add_task" placeholder='Enter a title for this card'/>`,
        );
      ...
      container.addEventListener("dragover", (e) => {
        ...
        if (draggingElement) {
          if (afterElement == null) {
            const addAnotherCardBtn =
              container.querySelector(".add_another_card");
  ...
  getDragAfterElement(container, y) {
    const draggableElements = [
      ...container.querySelectorAll(".wrapper:not(.dragging)"),
    ];
